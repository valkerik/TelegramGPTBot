...
public class GptService {
    ...
    public synchronized String SendMessage(Update update) {
        ...
        try {
            ...
            e.printStackTrace();
           log.error("Извините, что-то пошло не так" + "Ошибка: " + e.getMessage());
            return "Извините, что-то пошло не так. Попробуйте придерживаться таймаута между сообщениями не менее 50сек" +
                    "(Идет процесс отладки этой ошибки разработчиком)";
        }
    }
